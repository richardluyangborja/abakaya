...
function countConsonants(word: string) {
  return word
    .toUpperCase()
    .split("")
    .filter((char) => /[A-Z]/.test(char) && !vowels.includes(char)).length;
}
...
export default function KatinigQuiz() {
  ...
  const setupTurn = () => {
    ...
    const consonantCount = countConsonants(randomWord);

    setWord(randomWord);
    setCorrectAnswer(consonantCount);
    setChoices(generateChoices(consonantCount));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#E3BD72" }}>
      ...
      {/* TITLE */}
      <Text style={styles.title}>Katinig</Text>
      ...
      {/* QUESTION */}
      <Text style={styles.question}>Ilan ang katinig sa salitang ito?</Text>
